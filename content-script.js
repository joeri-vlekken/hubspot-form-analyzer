      ...
      forms.forEach((form) => {
        ...
        if (form.querySelectorAll(".hs-form-debug-header").length === 0) {
          ...
          chrome.storage.sync.get(["hsfa_showEdit"], function (result) {
            ...
            form.insertAdjacentHTML("afterbegin", header);
          });
        }
        ...
        fields.forEach((field) => {
          ...
          // Add the debug-show class to visualize the form row and field.
...
const callback = (mutations) => {
  mutations.forEach((mutation) => {
    if (mutation.type === "childList") {
      const listValues = Array.from(targetNode.children).map(
        (node) => node.innerHTML
      );
      if (
